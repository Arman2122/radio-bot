const { CHANNEL, STATUS, LIVE, CHANNEL_NAME } = require("../config.json");
...
module.exports = async (client, process) => {
  ...
  // Send Message When Bot Online 
  const guild = client.guilds.cache.get("490598681691553802");
  client.channels.cache.get("631878009779126282").send(
    new MessageEmbed()
      .setColor('RANDOM')
      .setTitle(CHANNEL_NAME)
      .setAuthor(guild.name, guild.iconURL(), "https://discord.gg/3ayHsTk")
      .setURL(LIVE)
      .setDescription("Status : <:online:893100969666158672> Playing")
      .setFooter ("24 /7 Radio", client.user.displayAvatarURL())
      .setTimestamp()
    );
